--[[
 ...
 function GuildQinglongLayer:ctor(data) 
    ...
    -- 输出排行 
  	...
  	extraRewardBtn:addHandleOfControlEvent(function(eventName, sender)
        	...
        	self:addChild(require("game.Worldboss.WorldBossExtraRewardLayer").new(data_boss_qinglong_boss_qinglong, confirmFunc), MAX_ZORDER)
        end, CCControlEventTouchUpInside) 
 ...
 function GuildQinglongLayer:updateLevel(level, currentUnionMoney) 
    ...
    if self._levelLbl ~= nil then 
        self._levelLbl:setString("LV." .. tostring(self._level)) 
    end 

    if self._curCoinLbl ~= nil then 
        self._curCoinLbl:setString(tostring(self._currentUnionMoney)) 
    end 
 ...
 function GuildQinglongLayer:createAllLbl() 
    ...
    -- 当前等级 
    self._levelLbl = self:createShadowLbl("LV." .. tostring(self._level), yColor, self._rootnode["cur_level_lbl"]) 
    ...
    self._curCoinLbl = self:createShadowLbl(tostring(self._currentUnionMoney), FONT_COLOR.WHITE, self._rootnode["cur_coin_lbl"])  
