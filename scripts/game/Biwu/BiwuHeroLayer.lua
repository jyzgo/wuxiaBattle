...
function BiwuHeroLayer:setUpLabelView()
	...
	local function createAddBtn(node)
		...
		buyBtn:setPosition(cc.p(node:getContentSize().width - 10,node:getContentSize().height/2))
	...
	--底部刷新按钮
	--按钮
    self.refreshBtn = cc.ui.UIPushButton.new(ReFreshBtnRes)
        :onButtonClicked(function()
        	GameAudio.playSound(ResMgr.getSFX(SFX_NAME.u_yeqian))
        	self:_getRefreshData()
        end)
        :pos( self:getContentSize().width/2 , self:getContentSize().height * 0.2)
        :addTo(self)
...
function BiwuHeroLayer:creatDislabel(titleicon,count,fontType)
	...
	local countLabel = ui.newTTFLabelWithShadow({text = count, 
...
function BiwuHeroLayer:createHeros(pos,index)
	...
	local disLabel   = ui.newTTFLabelWithShadow({  text = "", 
											size = 24,
											color = FONT_COLOR.YELLOW,
									        align= ui.TEXT_ALIGN_CENTE,
									        shadowColor = ccc3(0,0,0),
...
function BiwuHeroLayer:refreshHeroByPos(pos,data)
	...
	if not self._isWeekDay then
		_levelDis = { 
						{ dis = "可获得\n少量积分", font = "fonts/font_yellow_brown_num.fnt",color = FONT_COLOR.YELLOW},
						{ dis = "可获得\n一般积分", font = "fonts/font_yellow_brown_num.fnt",color = FONT_COLOR.BLUE},
						{ dis = "可获得\n极高积分", font = "fonts/font_yellow_brown_num.fnt",color = FONT_COLOR.GREEN}
					}
	else
		_levelDis = { 
	...
	if data.faction ~= "" then
		...
		bng:setScaleY(1)
		bng:setPositionY(bng:getPositionY() + 13)
...
function BiwuHeroLayer:countDownLogic( ... )
	self.refreshBtn:setButtonEnabled(self.dataCenter.colltime == 0)
