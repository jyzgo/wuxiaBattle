--[[
     		...
     		callback = function(data)
     			...
     			if data["0"] ~= "" then 
                    ...
                    self._bReqEndResult = false  
     				...
     				self._endTime = data["1"].endTime or 0 
 ...
 function WorldBossScene:ctor()
    ...
    self._rootnode["extraRewardBtn"]:addHandleOfControlEvent(function(eventName, sender) 
            GameAudio.playSound(ResMgr.getSFX(SFX_NAME.u_queding)) 
            self:addChild(require("game.Worldboss.WorldBossExtraRewardLayer").new(data_boss_boss), MAX_ZORDER)
        end, CCControlEventTouchUpInside) 
 ...
 function WorldBossScene:initData(data) 
 	...
 	-- 血量
    self._bossLife = stateObj.life 
 	self._rootnode["blood_lbl"]:setString(tostring(self._bossLife) .. "/" .. tostring(stateObj.lifeTotal)) 
 	-- 血量条
    local percent = self._bossLife/stateObj.lifeTotal 
